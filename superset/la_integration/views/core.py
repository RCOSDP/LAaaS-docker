...
class Superset(BaseSupersetView):
    ...
    def ini_reader(self):
        ...
        self.config.read(os.path.abspath('/venv/lib/python2.7/site-packages/superset/views/settings.ini'))

    @has_access
    @expose('/models')
    def models(self):       
        self.ini_reader() 
        mongodb_dbname = self.config.get('MongoDB','dbname')
        mongodb_hostname = self.config.get('MongoDB','hostname')
        mongodb_port = self.config.get('MongoDB','port')
        main_dir = self.config.get('Shared_Directory','main_dir')
        client = MongoClient('mongodb://'+mongodb_hostname+':'+mongodb_port+'/'+mongodb_dbname)
        db = client.learninglocker
        course_names = db.statements.distinct('statement.context.contextActivities.grouping.definition.name.en')
        BASE_FILES = main_dir+'/*.ipynb'
        files_path = []
        for files in glob.glob(BASE_FILES):
            filename = files.split('/')[-1].split('.')[0] + '.ipynb'
            files_path.append(filename)
        return self.render_template('superset/models.html', files_path= files_path, course_names=course_names)

    @has_access
    @expose('/execute')
    def execute(self):
        self.ini_reader()
        file = request.args.get('file_select')
        course = request.args.get('course_select')
        insertion_method = request.args.get('insertion_method')
        main_dir = self.config.get('Shared_Directory','main_dir')
        backup_ipynb_dir = self.config.get('Shared_Directory','backup_ipynb_dir')
        csv_dir = self.config.get('Shared_Directory','csv_dir')
        backup_csv_dir = self.config.get('Shared_Directory','backup_csv_dir')
        env_str = "ENV_VAR=\"%s\" " % course
        env_str += 'jupyter nbconvert --to notebook --execute ' + main_dir + file +' --output-dir ' + main_dir
        env_str = env_str.encode("utf-8")
        os.system(env_str)
        ipynb_backup_file_command = 'cp -a ' + main_dir + '/' + file + '  ' + backup_ipynb_dir + '/' + file + '"-$(date +"%m-%d-%y-%r")"'
        os.system(ipynb_backup_file_command)
        recent_csv_filename = max(glob.iglob(csv_dir+"*.csv"), key=os.path.getmtime)
        csv_filename = recent_csv_filename.split('/')[-1]
        csv_backup_file_command = 'cp -a ' + csv_dir + csv_filename + '  '  + backup_csv_dir + csv_filename + '"-$(date +"%m-%d-%y-%r")"'
        os.system(csv_backup_file_command)
        self.populate_table(recent_csv_filename, insertion_method)
        result = "Python Program Executed Successfully!!!! check configured dashboard!!"
        return self.render_template('superset/execute.html', file= file, result=result, selected_course=course)
    ...
    def source(self, file):
        self.ini_reader()
        main_dir = self.config.get('Shared_Directory','main_dir')
        filename = main_dir + '/' + file
        with open(filename, 'r') as myfile:
            data=myfile.readlines()
        return self.render_template('superset/source.html',file=filename, data=data)

    def populate_table(self, csv_filename, insertion_method):
        self.ini_reader()
        psql_dbname = self.config.get('PostgreSQL','dbname')
        psql_username = self.config.get('PostgreSQL','username')
        psql_password = self.config.get('PostgreSQL','password')
        psql_hostname = self.config.get('PostgreSQL','hostname')
        psql_port = self.config.get('PostgreSQL','port')
        conn = psycopg2.connect(database=psql_dbname, user=psql_username, password=psql_password, host=psql_hostname,port=int(psql_port))
        cursor = conn.cursor()
        cursor.execute("""SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'""")
        list_table_names = list(sum(cursor.fetchall(), ()))
        table_name = csv_filename.split('/')[-1].split('.')[0]
        if table_name in list_table_names:
            original_csv_header = pandas.read_csv(csv_filename).columns.tolist()
            csv_data = pandas.read_csv(csv_filename, skiprows=[0], names=original_csv_header,
                                       infer_datetime_format=True)            
            engine = create_engine('postgresql://'+psql_username+':'+psql_password+'@'+psql_hostname+'/'+psql_dbname)
            conn = engine.raw_connection()
            if insertion_method == 'replace':
                csv_data.to_sql(table_name, engine, if_exists='replace', chunksize=1000, index=False)
                conn.commit()
            elif insertion_method == 'append':
                csv_data.to_sql(table_name, engine, if_exists='append', chunksize=1000, index=False)
                conn.commit()
        else:
            original_csv_header = pandas.read_csv(csv_filename).columns.tolist()
            csv_data = pandas.read_csv(csv_filename, skiprows=[0], names=original_csv_header,
            ...
            engine = create_engine('postgresql://'+psql_username+':'+psql_password+'@'+psql_hostname+'/'+psql_dbname)
            conn = engine.raw_connection()
            csv_data.to_sql(table_name, engine, chunksize=1000, index=False)
