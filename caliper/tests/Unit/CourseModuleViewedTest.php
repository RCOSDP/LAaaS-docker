...
final class CourseModuleViewedTest extends TestCase
{
    ...
    {
        $this->assertTrue(is_supported('\mod_forum\event\course_module_viewed'));
    }

    public function testTranslatorShouldImplementAbstractMethods()
    {
        $event = Event::where('eventname', '\mod_forum\event\course_module_viewed')->first();
    ...
    public function testCanGetCompiler($interProd)
    {
        ...
        $this->assertEquals('App\Profiles\CourseModuleViewed', get_class($product));
    ...
    public function testObjectValues($dependValues)
    {
        ...
        $this->assertEquals(
            $interProd->getCategory(),
            $product->getObject()->getIsPartOf()->getCategory()
        );
        $this->assertEquals(
