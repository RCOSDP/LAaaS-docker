...
----
$ docker exec learning-analytics-db psql -U learning_analytics learning_analytics -c "SELECT count(*) FROM xapi_records_processed;"
----

また、スクリプトのバージョンアップ等に伴い処理済みのMoodleログを再度処理したい場合は、以下のコマンドで変換処理の実行履歴を削除する。

----
$ docker exec learning-analytics-db psql -U learning_analytics learning_analytics -c "DELETE FROM xapi_records_processed;"
----

<<<
=== Caliperステートメント変換
==== ログデータベースの起動
`compose.yaml` の `caliper-log-db` にデフォルトの設定が記載されている。変更する場合、`caliper/.env` 内の `DB_LOG_*` の項目も合わせて変更する。
...
設定完了後、 `caliper-log-db` を起動する。

----
$ docker compose up -d caliper-log-db
----

<<<
[[caliper_settings]]
==== 初期設定
`openlrw-mongo` のMongoDBに接続する。ログイン情報は `openlrw/.env` に従う。

----
$ docker exec -it openlrw-mongo bash
...
`openlrw-mongo` に接続し、データベースに送出したステートメントが登録されていることを確認する。
