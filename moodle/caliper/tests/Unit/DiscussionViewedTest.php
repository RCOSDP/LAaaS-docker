...
final class DiscussionViewedTest extends TestCase
{
    ...
    {
        $this->assertTrue(is_supported('\mod_forum\event\discussion_viewed'));
    }

    public function testTranslatorShouldImplementAbstractMethods()
    {
        $event = Event::where('eventname', '\mod_forum\event\discussion_viewed')->first();
    ...
    public function testCanGetCompiler($interProd)
    {
        ...
        $this->assertEquals('App\Profiles\DiscussionViewed', get_class($product));
    ...
    public function testObjectValues($dependValues)
    {
        ...
        $this->assertEquals(
            $interProd->getPartOfId(),
            $product->getObject()->getIsPartOf()->getId()
        );
        $this->assertEquals(
            $interProd->getPartOf()->name,
        ...
        $this->assertEquals(
            env('APP_URL')
            . '/course/view.php?id='
            . $interProd->getIsPartOfCourse()->id,
            $product->getObject()->getIsPartOf()->getIsPartOf()->getId()
        );
        $this->assertEquals(
            $interProd->getIsPartOfCourse()->fullname,
            $product->getObject()->getIsPartOf()->getIsPartOf()->getName()
        );
        $this->assertEquals(
            $product->getObject()->getIsPartOf()->getIsPartOf()->getType(),
            'CourseSection'
        );
    }
