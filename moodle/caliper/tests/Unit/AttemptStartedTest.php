...
final class AttemptStartedTest extends TestCase
{
    ...
    {
        $this->assertTrue(is_supported('\mod_quiz\event\attempt_started'));
    }

    public function testTranslatorShouldImplementAbstractMethods()
    {
        $event = Event::where('eventname', '\mod_quiz\event\attempt_started')->first();
    ...
    public function testCanGetCompiler($interProd)
    {
        ...
        $this->assertEquals('App\Profiles\AttemptStarted', get_class($product));
    ...
    public function testObjectValues($dependValues)
    {
        ...
        $this->assertEquals(
            $interProd->getQuiz()->grade,
            $product->getObject()->getMaxScore()
        );
        $this->assertEquals(
            $partOfId = env('APP_URL')
            . '/course/view.php?id='
